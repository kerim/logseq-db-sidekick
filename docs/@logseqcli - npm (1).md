...
This section assumes you have installed the CLI from npm or via the [dev setup](https://www.npmjs.com/package/@logseq/cli#setup). If you haven't, substitute `node cli.mjs` for `logseq` e.g. `node.cli.mjs -h`.
...
Most of this library is also compatible with ClojureScript for use on the frontend. This library follows the practices that [the Logseq frontend follows](https://github.com/logseq/logseq/blob/HEAD/deps/cli/docs/dev-practices.md). Most of the same linters are used, with configurations that are specific to this library. See [this library's CI file](https://github.com/logseq/logseq/blob/HEAD/deps/cli/.github/workflows/cli.yml) for linting examples.
...
Testing is done with nbb-logseq and [nbb-test-runner](https://github.com/nextjournal/nbb-test-runner). Some basic usage:
...
See [standard nbb/cljs library advice in graph-parser](https://github.com/logseq/logseq/blob/HEAD/deps/cli/deps/graph-parser/README.md#managing-dependencies).