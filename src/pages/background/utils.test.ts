...
describe('renderBlock', () => {
  ...
  test('should format link as markdown', () => {
    ...
    const renderBlock = blockRending({
      clipNoteTemplate: clipNoteTemplate,
      title: 'Logseq DB Sidekick',
      url: 'https://logseq-db-sidekick.eindex.me',
      time: time,
      data: '',
      preferredDateFormat: 'yyyy-MM-dd',
    });
    ...
    expect(renderBlock).toEqual(
      '[Logseq DB Sidekick](https://logseq-db-sidekick.eindex.me)',
    );
  });
...
describe('test logseq_sidekick', () => {
  it('test logseq-sidekick.logseqEscape', () => {
    let str = '[[a]]';
